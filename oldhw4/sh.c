...
void builtin(char** argv) {
	...
	if (!strcmp(argv[0], "cd")) {
		if (argv[1] == NULL) { // cd by itself
			chdir(getenv("HOME")); // get full string i.e., /home/user/
			updatePWDs();
		}
		else {
			chdir(argv[1]); // please use full string cd command.. not "cd GitHub".. "cd /home/user/GitHub". and yes this does work.
			updatePWDs();
		}
	}
