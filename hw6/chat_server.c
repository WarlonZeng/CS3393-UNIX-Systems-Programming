...
char *COMMANDS = "Type 'exit' to exit, or press CTRL-C\r\n";
...
/* welcomes the client with welcome message and commands */
void welcomeClient(client_t *client) {
	if (write(client->client_sockfd, WELCOME_MSG, strlen(WELCOME_MSG)) < 0) {
		perror("Error: Failed to write to client");
	}
	if (write(client->client_sockfd, COMMANDS, strlen(COMMANDS)) < 0) {
		perror("Error: Failed to write to client");
	}
}
...
void *consumerThread(void *args) {
	while (1) {
		pthread_mutex_lock(&queued_mutex);
		while (queued == 0) 
			pthread_cond_wait(&cond, &queued_mutex);
		flushMessageQueue();
		queued = 1;
		pthread_mutex_unlock(&queued_mutex);
	}
	pthread_exit(0);
}
...
void *clientCloneFactory(void *args) {
	...
	connected++;
	pthread_mutex_unlock(&connected_mutex); /* it is a shared variable */
	...
	while (read(client->client_sockfd, buf, BUF_LEN) > 0) {
		...
		memset(buf, 0, sizeof(buf)); /* reset buffer for every read, it appends to it */
		pthread_mutex_unlock(&queued_mutex);
	}
